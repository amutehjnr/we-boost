...
import API from "../../../lib/api"; // your API instance with token
import {
  FaInstagram,
  FaFacebook,
  FaTiktok,
...
export default function AvailableTask() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const res = await axios.get("/tasks/my-tasks?limit=10");
        setTasks(res.data.tasks || []);
      } catch (err) {
        console.error(err);
        setError(
          err.response?.data?.message || "Failed to load tasks. Try again."
        );
      } finally {
        setLoading(false);
      }
        ...
        {tasks.map((task) => (
          ...
          >
            <div className="flex items-center gap-4 mb-4">
              {platformIcons[task.platform] || <span className="text-3xl">🎯</span>}
              <div>
